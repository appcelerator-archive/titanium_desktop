...
{
#if defined(OS_WIN32)
	AutoPtr<Process> process = new ProcessWin();
#else
	AutoPtr<Process> process = new PosixProcess();
#endif
	return process;
}
...
Process::Process()
	...
	, onRead(0)
	, onExit(0)
	, exitMonitorAdapter(new Poco::RunnableAdapter<Process>(
...
void Process::SetStdin(AutoPtr<Pipe> newStdin)
{
	if (running)
	{
...
void Process::SetStdout(AutoPtr<Pipe> newStdout)
{
	if (running)
	{
...
void Process::SetStderr(AutoPtr<Pipe> newStderr)
{
	if (running)
	{
