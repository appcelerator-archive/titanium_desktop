...
PopupDialog::PopupDialog(HWND windowHandle)
...
/*static*/
INT_PTR CALLBACK PopupDialog::Callback(HWND dialog, UINT message, WPARAM wParam, LPARAM lParam)
{
	switch (message)
	{
		case WM_INITDIALOG:
		...
		case WM_COMMAND:
			...
			{
				PopupDialog::HandleOKClick(dialog);
				EndDialog(dialog, 0);
			}
...
int PopupDialog::CountMatches(std::string& message, const char *token)
{
	int matches = 0;
...
BOOL PopupDialog::ShowMessageBox(HWND hwnd)
{
	BOOL fSuccess = FALSE;
	HDC hdc = GetDC(hwnd);
