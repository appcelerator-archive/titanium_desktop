...
{
	MenuItemGtk* item = static_cast<MenuItemGtk*>(data);
	if (item->IsCheck())
	{
...
MenuItemGtk::MenuItemGtk(MenuItemType type)
	: MenuItem(type)
	, oldSubmenu(0)
{
}

MenuItemGtk::~MenuItemGtk()
{
}

void MenuItemGtk::SetLabelImpl(std::string newLabel)
{
...
void MenuItemGtk::SetIconImpl(std::string newIconPath)
{
...
void MenuItemGtk::SetStateImpl(bool newState)
{
	if (!this->IsCheck())
		return;
...
void MenuItemGtk::SetSubmenuImpl(AutoPtr<Menu> newSubmenu)
{
	if (this->IsSeparator())
		return;
	...
	this->oldSubmenu = newSubmenu.cast<MenuGtk>();
}

void MenuItemGtk::SetEnabledImpl(bool enabled)
{
	if (this->type == SEPARATOR)
		return;
...
void MenuItemGtk::SetNativeItemIcon(GtkMenuItem* nativeItem, std::string& newIconPath)
{
...
void MenuItemGtk::ReplaceNativeItem(GtkMenuItem* nativeItem, GtkMenuItem* newNativeItem)
{
...
void MenuItemGtk::SetNativeItemSubmenu(GtkMenuItem* nativeItem, AutoPtr<Menu> newSubmenu)
{
	...
	AutoPtr<MenuGtk> newGtkSubmenu = newSubmenu.cast<MenuGtk>();
...
GtkMenuItem* MenuItemGtk::CreateNative(bool registerNative)
{
...
void MenuItemGtk::DestroyNative(GtkMenuItem* nativeItem)
{
	std::vector<GtkMenuItem*>::iterator i = this->nativeItems.begin();
	while (i != this->nativeItems.end())
	{
