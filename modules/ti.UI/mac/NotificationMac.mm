...
static map<string, KMethodRef> callbackMap;
static Mutex callbackMapMutex;
...
static string GetClickContext(CFDictionaryRef dictionary)
{
...
{
    string context(GetClickContext(userInfo));
    if (context.empty() && callbackMap.find(context) == callbackMap.end())
        return;

    {
        Mutex::ScopedLock lock(callbackMapMutex);
...
bool Notification::InitializeImpl()
{
    ...
    if (connection)
    {
        ...
        @catch(NSException *e)
        {
            kroll::Logger::Get("Notification.Notification")->Error(
...
bool Notification::ShowImpl()
{
    ...
    {
        Mutex::ScopedLock lock(callbackMapMutex);
        string clickContextString(CFStringToUTF8(clickContext.get()));
